...
class Logger:
    def __init__(self, log_each=10, initial_epoch=-1):
        self.loading_bar = LoadingBar(length=25)
    ...
    def __call__(self, model, loss, accuracy, learning_rate=None):
        if self.is_train:
    ...
    def flush(self):
        if self.is_train:
            ...
            print(
                f"\r┃{self.epoch+1:12d}  ┃{loss:12.4f}  |{100*accuracy:10.2f} %  ┃{self.learning_rate:12.3e}  |{self._time():>12}  ┃",
                end="",
                flush=True
            )
        else:
    ...
    def _train_step(self, model, loss, accuracy, learning_rate):
        ...
        if self.step % self.log_each == self.log_each - 1:
            ...
            print(
                f"\r┃{self.epoch+1:12d}  ┃{loss:12.4f}  │{100*accuracy:10.2f} %  ┃{learning_rate:12.3e}  │{self._time():>12}  {self.loading_bar(progress)}",
                end="",
                flush=True,
            )
